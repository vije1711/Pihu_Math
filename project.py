...
class GUI_Exam(Exam):
    ...
    def launch_progress_dashboard(self):
        ...
        log_df["_start"] = log_df["Start Time"].astype(str).str[-8:]
        idx_df["_start"] = idx_df["Start Time"].astype(str).str[-8:]
        log_df = log_df.merge(idx_df[["_start", "Session Number"]], on="_start", how="left")
        log_df.drop(columns=["_start"], inplace=True)
